sap.ui.define([
	...
	return BaseObject.extend("ZGH.model.GroupSortState", {
		 ...
		 * @class
		 * @public
		 * @alias ZGH.model.GroupSortState
		 */
		...
		/**
		 * Sorts by LINE_DESCRIPTION, or by WTR_LVL
		 *
		...
		/**
		 * Groups by WTR_LVL, or resets the grouping for the key "None"
		 *
		 * @param sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function(sKey) {
			...
			if (sKey === "WTR_LVL") {
				...
				this._oViewModel.setProperty("/sortBy", "WTR_LVL");

				aSorters.push(
					new Sorter("WTR_LVL", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "LINE_DESCRIPTION");
			}
