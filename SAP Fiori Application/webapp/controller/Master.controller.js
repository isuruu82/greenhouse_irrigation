...
sap.ui.define([
	...
	return BaseController.extend("ZGH.controller.Master", {
		...
		onInit: function() {
			...
			this._oGroupSortState = new GroupSortState(oViewModel, grouper.WTR_LVL(this.getResourceBundle()));
		...
		onSearch: function(oEvent) {
			...
			if (sQuery) {
				this._oListFilterState.aSearch = [new Filter("LINE_DESCRIPTION", FilterOperator.Contains, sQuery)];
			} else {
		...
		onOpenViewSettings: function() {
			if (!this._oViewSettingsDialog) {
				this._oViewSettingsDialog = sap.ui.xmlfragment("ZGH.view.ViewSettingsDialog", this);
		...
		onConfirmViewSettingsDialog: function(oEvent) {
			...
			aFilterItems.forEach(function(oItem) {
				switch (oItem.getKey()) {
					case "Filter1":
						aFilters.push(new Filter("WTR_LVL", FilterOperator.LE, 100));
						break;
					case "Filter2":
						aFilters.push(new Filter("WTR_LVL", FilterOperator.GT, 100));
						break;
					default:
						break;
				}
				...
				function(mParams) {
					...
					if (!mParams.list.getSelectedItem()) {
						this.getRouter().navTo("object", {
							PLANT_LINE_ID: encodeURIComponent(mParams.firstListitem.getBindingContext().getProperty("PLANT_LINE_ID"))
						}, true);
					}
				}.bind(this),
				function(mParams) {
		...
		_showDetail: function(oItem) {
			...
			this.getRouter().navTo("object", {
				PLANT_LINE_ID: encodeURIComponent(oItem.getBindingContext().getProperty("PLANT_LINE_ID"))
			}, bReplace);
		},
