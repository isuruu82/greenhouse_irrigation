			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function(oObject) {
											return !oList.getItems().some(function(oListItem) {
												return oListItem.getBindingContext().getProperty("PLANT_LINE_ID") === oObject.PLANT_LINE_ID;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].PLANT_LINE_ID;
									} else {
										sCurrentId = aItemsNotInTheList[0].PLANT_LINE_ID;
									}
				...
				iRememberTheListItem: function(oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("PLANT_LINE_ID"),
						title: oBindingContext.getProperty("LINE_DESCRIPTION")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function() {
					return this.waitFor({
						...
						success: function() {
							Opa5.assert.ok(true, "The list header displays 'PL_LINE (0)'");
						},
				...
				theListShouldBeSortedAscendingOnUnitNumber: function() {
					return this.theListShouldBeSortedAscendingOnField("WTR_LVL");
				},

				theListShouldBeSortedAscendingOnName: function() {
					return this.theListShouldBeSortedAscendingOnField("LINE_DESCRIPTION");
				},
				...
				theListShouldHaveAllEntries: function() {
					...
					// retrieve all PL_LINE to be able to check for the total amount
